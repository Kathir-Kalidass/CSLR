...
from utils.misc import load_state_dict_for_vit
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.fc_norm is not None:
            if self.head_variant == 'default':
                return self.fc_norm(x.mean(1))
            elif self.head_variant == 'later_mean':
                return self.fc_norm(x)
        else:
            return x[:, 0]

    def forward(self, sgn_videos):
        sgn_videos = self.forward_features(sgn_videos)
        logits = self.head(sgn_videos)
        if self.head_variant == 'later_mean':
            logits = logits.mean(dim=1)
        return {'gloss_feature': sgn_videos,
                'gloss_feature_norm': F.normalize(sgn_videos, dim=-1),
                'gloss_logits': logits, 
...
def build_vit(num_cls, num_frames, cfg, in_channels=3, img_size=224):
    variant = cfg.get('variant', 'vit_base_patch16_224')
    pretrained = cfg.get('pretrained_ckpt', None)
    head_variant = cfg.get('head_variant', 'default')
    model = create_model(
        variant,
        pretrained=False,
        num_classes=num_cls,
    ...
    if pretrained is not None:
        ...
        load_state_dict_for_vit(model, checkpoint_model, prefix='')
    return model